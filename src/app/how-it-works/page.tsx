        ...
        <div className="section-container grid gap-4">
          {steps.map((item) => (
            <div key={item.step} className="surface flex flex-col gap-5 p-6 md:flex-row md:items-start">
              <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-accent text-lg font-black text-parchment">
                {item.step}
              </div>
              <div>
                <h2 className="text-2xl">{item.title}</h2>
                <p className="mt-3 max-w-3xl leading-8 text-ink-muted">{item.body}</p>
              </div>
            </div>
          ))}
        </div>
